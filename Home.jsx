...
function Home() {
  ...
  useEffect(() => {
    ...
    const onData = (snapshot) => {
      ...
      setBinData(data);
      updatePieChart(data);
      if (data && data.percentage <= 90) { // Check if the bin is full
        axios
        ...
        datasets: [
          {
            ...
            // backgroundColor:["#A100FFFF","#119CFDFF"],
            hoverOffset: 4,
          },
        ],
      },
        ...
        <div className="col-md-6" style={{ paddingLeft: "30px", marginTop:"250px"}}>
          {binData && (
            <div>
              ...
              <p style={style.info}>Level: <span style={style.boldGreen}>{binData.percentage}</span></p>
            </div>
          )}
        </div>
      </div>
